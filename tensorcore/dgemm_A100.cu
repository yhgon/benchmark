...
printf("===================================================================\n");
printf(" DGEMM calculation with NVIDIA A100 GPU\n");
printf(" Orignal      : By Hyungon  Ryu @ NVAITC, 30 Aug 2018\n");
printf(" Modification : By Youngwoo Kim @ ETRI  , 04 Apr 2022\n");
printf("===================================================================\n");

if(argc<4){
	printf("===================================================================\n");
	printf(" plz use matrix size M=N=K with command  ./dgemm  8192  \n");
...
#define num_clock 1410 // A100=1410 (A100 40G PCIe) // V100=1530 (V100 16GB SMX)
#define num_SM 108 // A100=108 // V100=80
#define num_CUDA 64 // A100 ??
#define num_FMA 2
...
#define TOTAL_OP2 (MATRIX_M*MATRIX_N) * (2*MATRIX_K+2) 
   ...
   // Use tensor cores ---------
   printf("        - use TENSOR cores for A100 ...\n");
   ...
   cublasDgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, 
    ...
    printf(" Step7.  Running with cuBLAS... dgemm\n");
  ...
  double RMAX = (double) ( ((double)TOTAL_OP / (double) (1000000) ) / ((double) cublasTime_fp64)/1000 );
   ...
   printf("FP64 RPeak: %.3f TFLOPS",  RPEAK*2 );  // for A100 Tensorcore
   printf(" DGEMM : %.3f TFLOPS\n",   RMAX  ); 

   //printf("Ratio of Real/Theoretic %.2f   \n",  RMAX/RPEAK );
   printf("Ratio of Real/Theoretic %.3f   \n",  RMAX/(RPEAK*2) ); // for A100 tensorcore
